module gem_com
  ...
  INTEGER :: timestep,im,jm,km,mykm,iseed,nrst,nfreq,isft,mynf,ifskp,iphbf,iapbf,idpbf,iseed2=0
  ...
  REAL,DIMENSION(:,:,:,:),allocatable :: den,den1
...
#ifdef RK4
  real,dimension(:,:,:),allocatable :: ex_3d
  real,dimension(:,:,:),allocatable :: ey_3d
  real,dimension(:,:,:),allocatable :: ez_3d
#ifdef USE_ACC_DECLARE
  !$acc declare create(ex_3d,ey_3d,ez_3d)
#endif
  ...
  !$acc declare create(nonlin)
  !$acc declare create(phi)
  !$acc declare create(ex,ey,ez)
  !$acc declare create(apar)
  !$acc declare create(dpdz,dadz,delbx,delby)
#endif
  ...
  REAL,DIMENSION(:,:),allocatable :: w2,w3,w0
#ifdef DIRECT_F
  REAL,DIMENSION(:,:),allocatable :: w5,pif0
!#ifdef KINETIC_ELECTRON
  REAL,DIMENSION(:),allocatable :: w5e,pif0e,w0e
!#endif
#endif
...
!$omp target enter data map(alloc:lr,mims,q)
#endif
    ...
    ALLOCATE( den(nsmx,0:nxpp,0:jmx,0:1),den1(nsmx,0:nxpp,0:jmx,0:1),dti(nsmx,0:nxpp,0:jmx,0:1), &
...
!$omp target enter data map(alloc:phi)
#endif
...
#ifdef RK4
    allocate(ex_3d(0:imx,0:jmx,0:kmx),ey_3d(0:imx,0:jmx,0:kmx),ez_3d(0:imx,0:jmx,0:kmx))
#ifndef USE_ACC_DECLARE
!!$acc enter data create(ex_3d,ey_3d,ez_3d)
!$omp target enter data map(alloc:ex_3d,ey_3d,ez_3d)
#endif
...
!$omp target enter data map(alloc:dpdz,dadz,delbx,delby)
#endif
    ...
    allocate( w2(1:mmx,nsmx),w3(1:mmx,nsmx),w0(1:mmx,nsmx))
#ifdef DIRECT_F
    allocate( w5(1:mmx,nsmx),pif0(1:mmx,nsmx))
#ifdef KINETIC_ELECTRON
    allocate( w0e(1:mmxe),w5e(1:mmxe),pif0e(1:mmxe))
#endif
#endif

!    !$acc enter data create(x2,y2,z2,u2,u3,x3,y3,z3,w2,w3)
!    !$acc enter data create(mu,xii,pzi,eki,z0i,u0i,w0)
    !$omp target enter data map(alloc:x2,y2,z2,u2,u3,x3,y3,z3,w2,w3)
    !$omp target enter data map(alloc:mu,xii,pzi,eki,z0i,u0i,w0)
#ifdef DIRECT_F
!    !$acc enter data create(w5,pif0)
    !$omp target enter data map(alloc:w5,pif0)
#ifdef KINETIC_ELECTRON
    !$acc enter data create(w5e,w0e,pif0e)
#endif
#endif


!#ifdef KINETIC_ELECTRON
!    !$acc enter data create(mue,xie,pze,eke,z0e,u0e,x2e,y2e,z2e,u2e,mue2)
!    !$acc enter data create(x3e,y3e,z3e,u3e,mue3,w2e,w3e,w000,w001,w010,w011,w100,w101,w110,w111)
!#endif
...
#ifdef KINETIC_ELECTRON
    !$acc enter data create(mue,xie,pze,eke,z0e,u0e,x2e,y2e,z2e,u2e,mue2)
    !$acc enter data create(x3e,y3e,z3e,u3e,mue3,w2e,w3e,w000,w001,w010,w011,w100,w101,w110,w111,ipass,index)
#endif
    !              Various diagnostic arrays and scalars
    !    plotting constants
