...
!  call ftcamp
  ...
  poisson_tot_tm=poisson2_end_tm-poisson2_start_tm
  ...
  grid1_tot_tm=grid1_end_tm0-grid1_start_tm0+grid1_end_tm1-grid1_start_tm1
  ...
  !!!new dignostics(htc)
  pmove_tm=push_tot_tm-ppush1_tot_tm-cpush1_tot_tm
  poisson1_tot_tm=poisson_tot_tm-poisson0_tot_tm
  push_tm=push_tot_tm-cpush1_tot_tm-ppush1_tot_tm 
  
  call MPI_REDUCE(totinit,init_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(totload,load_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  ...
  init_tm=init_tm/numprocs
  load_tm=load_tm/numprocs        
...
#endif
!  double precision :: start_time_ppush, openmp_runtime_ppush
...
!  !$omp target update from(z3)
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)then
     open(18, file='ppush_after_pmove', status='unknown',position='append')
!     write(18,*)'ppush_after_pmove',z3
     close(18)
  end if
...
subroutine cpush(n,ns)
  ...
  real :: new_f0,w4,dw
#endif
!  double precision :: start_time_cpush, openmp_runtime_cpush
...
subroutine grid1(ip,n,jump_opt)
  ...
  integer :: jump_opt
  integer :: mm_of_ns
!  double precision :: start_time_grid1, openmp_runtime_grid1
...
if(jump_opt==0)then
  ...
  if(idg.eq.0)write(*,*)'enter ion grid1',mm(1)
  do ns = 1,nsm
     den(ns,:,:,:)=0.
     jpar(ns,:,:,:)=0.
     myden = 0.
...
!  !$acc update device(w0(:,ns))
  !$omp target update to(w0(:,ns))
#ifdef DIRECT_F
  call pmove(pif0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
!  !$acc update device(pif0(:,ns))
  !$omp target update to(pif0(:,ns))
  call pmove(w5(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
!  !$acc update device(w5(:,ns))
  !$omp target update to(w5(:,ns))
#endif
  !     
  call end_pmove(ierr)
  mm(ns)=np_new
...
subroutine accumulate(n,ip)
  ...
  grid1_start_tm1=grid1_start_tm1+MPI_WTIME()
  call grid1(ip,n,1)
  grid1_end_tm1=grid1_end_tm1+MPI_WTIME()
  !$acc wait  
...
subroutine poisson(n,ip)
  ...
  real :: start_time_gkps, openmp_runtime_gkps

start_time_gkps = mpi_wtime()
...
!  !$acc update device(phi)
  !$omp target update to(phi)
  call MPI_BARRIER(MPI_COMM_WORLD,ierr) 
openmp_runtime_gkps=mpi_wtime() - start_time_gkps  
if(myid.eq.master)then
open(9, file='plot',status='unknown',position='append')
write(9,*)'OpenMP_runtime_gkps= ',openmp_runtime_gkps
close(9)
endif
...
!  !$acc update device(phi,ex,ey,ez) 
  !$omp target update to(phi,ex,ey,ez)
