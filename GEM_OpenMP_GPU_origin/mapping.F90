...
subroutine mapping_init
  ...
  if(myid==0)write(*,*)'before allocate in others'
  if(MyId .ne. 0)then
    allocate(grid%surf_len(grid%npsi_surf))
    allocate(grid%surf_idx(grid%surf_maxlen,grid%npsi_surf))
    grid%surf_idx=0
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after allocate in others'
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after allocate grid, surf_idx'
  if(MyId==0)then
    do i=1,n_n
  ...
  !3d density/field for XGC
  allocate(pot_density_3d(cce_all_surface_node_number,nphi))
  allocate(pot_field_3d(cce_all_node_number,nphi))
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after allocate pot','imx,jmx,kmx',imx,jmx,kmx,'cce_all_surface_node_number,nphi',cce_all_surface_node_number,nphi,'cce_all_node_number,nphi',cce_all_node_number,nphi
  density_3d=0.0
  field_3d=0.0
  pot_density_3d=0.0
  pot_field_3d=0.0  
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after mapping_init'
...
subroutine mapping_GEM_XGC(density_gem,density_xgc,den_tmp,grid,coef)
  ...
  if(MyId==0)then
    !mapping
    call mapping_GEM_XGC_core(density_gem,density_xgc,grid,coef)
  endif
...
subroutine mapping_GEM_XGC_new(density_gem,density_xgc,den_tmp,grid,coef)
  ...
  if(MyId==0)write(*,*)'after 3d GEM, begin mapping GEM XGC',MPI_WTIME()
  !mapping
  call mapping_GEM_XGC_core_new(density_gem,density_xgc,grid,coef)
  if(Myid==0)write(*,*)'after mapping GEM XGC',MPI_WTIME()
  call mpi_barrier(mpi_comm_world,ierr)
...
subroutine mapping_GEM_XGC_core(density_gem,density_xgc,grid,coef)
  ...
  do i=0,kmx
    z_gem(i)=lz/real(kmx)*real(i)
    k=int(z_gem(i)/delz)
    wz0=(real(k+1)*delz-z_gem(i))/delz
    wz1=1-wz0
...
subroutine mapping_GEM_XGC_core_new(density_gem,density_xgc,grid,coef)
  ...
  real, intent(in) :: density_gem(0:imx,0:jmx,0:kmx)
  real, intent(out) :: density_xgc(cce_all_surface_node_number,nphi)
  integer :: i,ii,j,iii,idx_coef,icount,ierr,j1,j2,k,node_start,local_start
  integer :: surf_len_local,surf_idx_local(grid%surf_maxlen)
  real :: w1,w2,wz0,wz1,th_tmp,theta_z_gem(0:kmx),density_gem_z(0:kmx), &
    theta_local(grid%surf_maxlen),theta_flux(grid%surf_maxlen),density_gem_z_xgc(0:jmx,grid%surf_maxlen), &
...
subroutine setup_GEM_XGC_coef(coef,grid)
  ...
  if(myid==0)write(*,*)'point 0, GEM XGC setup,',MPI_WTIME()
  !nwedge=lymult
  node_start=grid%surf_idx(1,cce_first_surface)
  ...
  if(myid==0)write(*,*)'point 1, GEM XGC steup,',MPI_WTIME()
  theta_grid(ntheta/2)=0.
  do i=ntheta/2+1,ntheta
     th_tmp=real(i-ntheta/2)*dth
     theta_grid(i)=th_tmp
     theta_grid(ntheta-i)=-th_tmp
  enddo
  if(myid==0)write(*,*)'point 2, GEM XGC steup,',MPI_WTIME()
  do ii=cce_first_surface,cce_last_surface
    ...
    call finalize_1d_interpolation(spl_thflx)
    if(myid==0 .and. ii==150)write(*,*)'point 2.2, GEM XGC steup,',MPI_WTIME()
    do i=1,surf_len_local
      do j=1,nphi
...
subroutine setup_GEM_XGC_coef_new(coef,grid)
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'point 3.0, GEM XGC setup,',MPI_WTIME()
