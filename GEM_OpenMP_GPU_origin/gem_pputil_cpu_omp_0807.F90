...
!    !$acc enter data create(s_counts)
    !$omp target enter data map(alloc:s_counts)
...
!    !$acc enter data create(r_displ)
  ...
  if(me==0)then
     open(18, file='xp_value', status='unknown',position='append')
     write(18,*) 'xp',xp
     close(18)
  end if
...
!    !$acc serial present(xp)
   !$omp target data map(to:xp)
    xpsize=size(xp)
   !$omp end target data
!$omp target update from (xpsize)
 if(me==0)then
     open(18, file='xp_size', status='unknown',position='append')
     write(18,*)'xpsize ',xpsize
     close(18)
  end if
!    !$acc end serial
...
!    !$acc enter data create(iz_arr)
   !$omp target enter data map(alloc:iz_arr)
    endif
     
!    if(me==0)write(*,*)'after test pmove 0',mpi_wtime() 
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='xp', status='unknown',position='append')
     write(18,*)'after0',xpsize
     close(18)
  end if
...
!    !$acc loop gang vector
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='dzz_lz', status='unknown',position='append')
     write(18,*) 'dzz_lz',dzz,lz,nvp,np,xpsize
     close(18)
  end if
...
!    !$acc wait
 ...
 call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='iz_arr', status='unknown',position='append')
     write(18,*)'iz_arr',iz_arr(1:np)
     close(18)
  end if
...
!    !$acc update host(iz_arr(1:np))
   !$omp target update from(iz_arr(1:np))
    if(me==0)write(*,*)'after test pmove 1.1',mpi_wtime()!,acc_is_present(s_counts),acc_is_present(iz_arr)
...
call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='s_counts_before', status='unknown',position='append')
     write(18,*)'GCLR,np',GCLR,np
     write(18,*)'s_counts',s_counts
     write(18,*)'s_counts(0:nvp-1)',s_counts(0:nvp-1)
     write(18,*)'iz_arr',iz_arr(1:np)
     close(18)
  end if
...
!   !$omp end target teams loop
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='s_counts', status='unknown',position='append')
     write(18,*)'s_counts',s_counts(0:nvp-1)
     close(18)
  end if
    ...
    if(me==0)write(*,*)'after test pmove 1.2',mpi_wtime()
...
!    !$acc end data
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='s_displ', status='unknown',position='append')
     write(18,*)'s_displ',s_displ
     close(18)
  end if
...
!       !$acc exit data delete(s_buf,ipsend,iphole,ipfill)
    !$omp target exit data map(delete:s_buf,ipsend,iphole,ipfill)
       DEALLOCATE(s_buf)
       DEALLOCATE(ipsend)
       DEALLOCATE(iphole)
       DEALLOCATE(ipfill)
...
!       !$acc enter data create(s_buf,ipsend,iphole,ipfill)
    ...
    !!!parallelized version (htc)
    !!$acc parallel loop independent present(iz_arr,s_displ,ipsend,iphole) 
    DO ip=1,np
       iz=iz_arr(ip)
       IF( iz .ne. GCLR ) THEN
          isb(iz) = isb(iz)+1
    ...
    !!$acc end parallel
    !
    if(me==0)write(*,*)'after test pmove 2.0',mpi_wtime()
    !!!update device(htc)
!    !$acc update device(ipsend,iphole)
    !$omp target update to(ipsend,iphole)
    !!!ipfill (htc)
   ...
   call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='pputil_iphole_before', status='unknown',position='append')
     write(18,*) 'nsize',nsize,np_new_ipf,ip_last_ipf
     write(18,*) 'iphole',iphole
     write(18,*) 'iz_arr',iz_arr
     close(18)
  end if
   ...
   !!$omp target teams loop bind(thread)
    ...
    !!!do nothing by default
       ...
       IF( iphole(ih_ipf) .LE. np_new_ipf ) THEN
          ...
          !!!search for xp(ip) from the end that is not a hole, need to be in
...
!    !$acc end serial
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='pputil_ipfill', status='unknown',position='append')
     write(18,*)'ipfill',ipfill
     close(18)
  end if
...
!       !$acc enter data create(r_buf)
       !$omp target enter data map(alloc:r_buf)
    ELSE IF ( SIZE(r_buf) .LT. nsize ) THEN
!       !$acc exit data delete(r_buf)
       !$omp target exit data map(delete:r_buf)
       DEALLOCATE(r_buf)
...
!!$acc loop independent  private(i,isrt,iend,isbuf)
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     open(18, file='xp_ipsend', status='unknown',position='append')
     write(18,*) 'before7',ipsend
     write(18,*) 's_displ',s_displ
     write(18,*) 's_counts',s_counts
     close(18)
  end if
...
!$omp target data map(to:xp,s_buf,ipsend,s_displ,s_counts)
!$omp target teams loop private(i,isrt,iend,isbuf)
    DO i=0,nvp-1
...
!$omp target data map(to:s_counts)
    s_buf_length=0
!!$acc loop gang vector reduction(+:s_buf_length) 
!$omp target teams loop reduction(+:s_buf_length)
    do i=0,nvp-1
...
!$omp target update from(s_buf(1:s_buf_length))
...
!    !$acc loop independent
  ...
  if(me==0)then
    ...
    !$omp target data map(to:xp,iphole,ipfill)
    !$omp target teams loop
    DO ih =1,nhole
 ...
 call mpi_barrier(mpi_comm_world,ierr)
  if(me==0)then
     ...
     close(18)
       ...
       !$omp target data map(to:xp,r_buf)
       !$omp target teams loop 
       do i=1,tot_count
          xp(i+np_new)=r_buf(i)
       enddo
