...
!$omp target update from(s_counts(0:nvp-1),iz_arr(1:np))

!    !$acc loop gang vector
!   !$omp target data map(to:s_counts(0:nvp-1),iz_arr(1:np))
...
!    !$acc serial present(xp,ipfill,iphole,iz_arr)
   !$omp target data map(to:nsize,np_new_ipf,ip_last_ipf)
   !$omp target teams loop bind(thread)
    DO ih_ipf=nsize,1,-1
