subroutine gkps_adiabatic_electron(nstep,ip)
  use gem_com
  use gem_equil
  use gem_fft_wrapper
  use assert_mod
  use batched_mod
  use iso_c_binding
  implicit none

  ! input variables
  integer :: nstep,ip

  ! global variables for matrix implement
!  integer, parameter :: wp = selected_real_kind(15,100)
  integer :: ifirst
  integer,dimension(:,:,:,:),allocatable :: ipiv,ipiv_zonal
  real,dimension(:,:,:),allocatable :: formphi
  complex,dimension(:,:,:,:),allocatable :: mx,mx_zonal

!  integer(kind=c_int),dimension(:,:,:),allocatable :: ipiv_zonal_3dim
!  complex(kind=wp),dimension(:,:,:),allocatable :: mx_zonal_3dim
  integer,dimension(:,:,:),allocatable :: ipiv_zonal_3dim
  complex,dimension(:,:,:),allocatable :: mx_zonal_3dim
  ! local variables for matrix implement
  integer :: i,j,k,l,m,n,i1,k1,m1,ns,ix,ikx,icount,INFO
  real :: r,th,wx0,wx1,wz0,wz1,kx1,kx2,ky,gam0,gam1,sgny, &
          bfldp,dydrp,qhatp,grp,gthp,gxdgyp,grdgtp
  real,dimension(1:5) :: b1,b2,ter
  real :: gamb1(1:5,0:imx-1,0:jcnt-1,0:imx-1,0:1),gamb2(1:5,0:imx-1,0:jcnt-1,0:imx-1,0:1), &
          jacob_theta(imx-1,0:1),jacob_theta_tmp(imx-1,0:1)
  complex :: mx0(imx-1,imx-1,0:jcnt-1,0:1),mx_tmp(imx-1,imx-1,0:jcnt-1,0:1),tmp
  character(len=70) fname
  character(len=5) holdmyid

  ! the local variables for field solver
  integer :: myj,ierr
  real :: u(0:imx,0:jmx,0:1),u_zonal(0:imx,0:jmx,0:1), &
          rho_zonal_tmp(0:imx,0:1),rho_zonal(0:imx,0:1),jacob_rho_tmp(0:imx,0:1),jacob_rho(0:imx,0:1),start_allreduce
  complex :: v(0:imx-1,0:jcnt-1,0:1),v_zonal(0:imx-1,0:jcnt-1,0:1),sl(1:imx-1,0:jcnt-1,0:1),temp3dxy(0:imx-1,0:jmx-1,0:1)
  complex :: sl_2dim(1:imx-1,1,1)
!  complex :: sl_2dim(1,1:imx-1,1)
  integer :: index_2dim 
  real :: theta
  complex :: z1, cz1


 
  integer(kind=c_size_t) :: strideA,strideB
  integer(kind=c_int), allocatable :: DINFO(:)
  character(kind=c_char) :: c_trans
  integer :: batchcount


  ! save the global variables
  save ifirst,mx,mx_zonal,ipiv,ipiv_zonal,formphi,mx_zonal_3dim,ipiv_zonal_3dim
  poisson0_start_tm=poisson0_start_tm+MPI_WTIME()
  poisson2_start_tm=poisson2_start_tm+MPI_WTIME()
  if(idg==1)write(*,*)'enter gkps'

  write(holdmyid,'(I5.5)') MyId
  fname='./matrix/'//'mx_phi_'//holdmyid

  ! form factors....
  if(ifirst.ne.-99)then
    allocate(mx(imx-1,imx-1,0:jcnt-1,0:1),mx_zonal(imx-1,imx-1,0:jcnt-1,0:1))
    allocate(ipiv(imx-1,imx-1,0:jcnt-1,0:1),ipiv_zonal(imx-1,imx-1,0:jcnt-1,0:1))
    allocate(formphi(0:imx-1,0:jcnt-1,0:1))
    ! allocate reshape array
    allocate(mx_zonal_3dim(imx-1,imx-1,1))
    allocate(ipiv_zonal_3dim(imx-1,imx-1,1))
    
    allocate(DINFO(batchcount))

    mx=0.0
    mx_zonal=0.0
    mx0=0.0
    mx_tmp=0.0

    ! restart the matrix
    if(iget.eq.1)then
      open(10000+MyId,file=fname,form='unformatted',status='old')
      read(10000+MyId)mx,mx_zonal,ipiv,ipiv_zonal
      close(10000+myid)
      goto 200
    end if

    ! construct the matrix for poisson solver
    do l=0,im-1
       do m=0,jcnt-1
          j=tclr*jcnt+m
          do i=0,im-1
             !j=tclr*jcnt+m
             r=lr0-lx/2+i*dx
             i1=int((r-rin)/dr)
             i1=min(i1,nr-1)
             wx0=(rin+(i1+1)*dr-r)/dr
             wx1=1.-wx0

             do ns=1,nsm
                ter(ns)=wx0*t0s(ns,i1)+wx1*t0s(ns,i1+1)
             enddo

             do n=0,1
                k=gclr*kcnt+n
                k1=int(k*dz/delz)
                k1=min(k1,ntheta-1)
                wz0=((k1+1)*delz-dz*k)/delz
                wz1=1-wz0
                th=wz0*thfnz(k1)+wz1*thfnz(k1+1)
                k=int((th+pi)/dth)
                k=min(k,ntheta-1)
                wz0=(-pi+(k+1)*dth-th)/dth
                wz1=1.-wz0
                bfldp=wx0*wz0*bfld(i1,k)+wx0*wz1*bfld(i1,k+1) &
                      +wx1*wz0*bfld(i1+1,k)+wx1*wz1*bfld(i1+1,k+1)
                dydrp=wx0*wz0*dydr(i1,k)+wx0*wz1*dydr(i1,k+1) &
                      +wx1*wz0*dydr(i1+1,k)+wx1*wz1*dydr(i1+1,k+1)
                qhatp=wx0*wz0*qhat(i1,k)+wx0*wz1*qhat(i1,k+1) &
                      +wx1*wz0*qhat(i1+1,k)+wx1*wz1*qhat(i1+1,k+1)
                grp=wx0*wz0*gr(i1,k)+wx0*wz1*gr(i1,k+1) &
                    +wx1*wz0*gr(i1+1,k)+wx1*wz1*gr(i1+1,k+1)
                gthp=wx0*wz0*gth(i1,k)+wx0*wz1*gth(i1,k+1) &
                     +wx1*wz0*gth(i1+1,k)+wx1*wz1*gth(i1+1,k+1)
                gxdgyp=wx0*wz0*gxdgy(i1,k)+wx0*wz1*gxdgy(i1,k+1) &
                       +wx1*wz0*gxdgy(i1+1,k)+wx1*wz1*gxdgy(i1+1,k+1)
                grdgtp=wx0*wz0*grdgt(i1,k)+wx0*wz1*grdgt(i1,k+1) &
                       +wx1*wz0*grdgt(i1+1,k)+wx1*wz1*grdgt(i1+1,k+1)
                 
                m1=mstart+int((real(m)+1.0)/2)
                if(m==0)m1=0
                sgny=isgnft(m)

                ky=sgny*2.*pi*real(m1)/ly
                kx1=pi*real(l)/lx
                kx2=-pi*real(l)/lx
                 
                ! the b+ and b- value
                do ns=1,nsm
                   b1(ns)=mims(ns)*(kx1*kx1*grp**2 + &
                          ky*ky*(dydrp**2*grp**2+(lr0/q0*qhatp*gthp)**2 &
                          +2*dydrp*lr0/q0*qhatp*grdgtp) &
                          +2*kx1*ky*gxdgyp)/(bfldp*bfldp)*ter(ns)/(q(ns)*q(ns))

                   b2(ns)=mims(ns)*(kx2*kx2*grp**2 + &
                          ky*ky*(dydrp**2*grp**2+(lr0/q0*qhatp*gthp)**2 &
                          +2*dydrp*lr0/q0*qhatp*grdgtp) &
                          +2*kx2*ky*gxdgyp)/(bfldp*bfldp)*ter(ns)/(q(ns)*q(ns))
                   ! construct the Gamma(b+) and Gamma(b-)
                   call srcbes(b1(ns),gam0,gam1)
                   gamb1(ns,l,m,i,n)=gam0
                   call srcbes(b2(ns),gam0,gam1)
                   gamb2(ns,l,m,i,n)=gam0
                enddo

                ! formfactor in gkps
                formphi(l,m,n) = 1./jmx
                if(abs(ky)>kycut)formphi(l,m,n) = 0.
                if(m1.ne.mlk.and.onemd==1)formphi(l,m,n) = 0.

             enddo
          enddo
       enddo
    enddo
    ! construct the matrix MX for Poisson sovler
!    !$omp target update from(bfldp,dydrp,qhatp,grp,gthp,gxdgyp,grdgtp,ky,kx1,kx2,b1,b2,gamb1,gamb2)  




!$acc data copyin(t0s,q,gt0e,gt0e,gn0s,gn0e,gamb2,gamb1,cn0s) copy(mx,mx0)
!$acc parallel 
!$acc loop gang vector private(ter) collapse(4) &
!$acc& private(k,j,i,ix,r,i1,wx0,wx1,ns,ikx,tmp) &
!$acc& private(theta,z1,cz1)
    do k=0,1
       do j=0,jcnt-1
          do i=1,imx-1
            do ix=1,imx-1
             r=lr0-lx/2+i*dx
             i1=int((r-rin)/dr)
             i1=min(i1,nr-1)
             wx0=(rin+(i1+1)*dr-r)/dr
             wx1=1.-wx0
             do ns=1,nsm
                ter(ns)=wx0*t0s(ns,i1)+wx1*t0s(ns,i1+1)
             enddo
             !do ix=1,imx-1
                !mx(i,ix,j,k)=0.0
                !mx0(i,ix,j,k)=0.0
                ! the adiabatic response of electron, the option is fradi
                if(i==ix)mx(i,ix,j,k) = fradi*gn0e(i)*cn0e/gt0e(i)
                do ikx=0,imx-1
                   do ns=1,nsm
                      ! the matrix MX with adiabatic electron
!                      tmp=q(ns)*sin(ix*ikx*pi/imx)* &
!                          ((1-gamb1(ns,ikx,j,i,k))*exp(IU*ikx*i*pi/imx)-&
!                          (1-gamb2(ns,ikx,j,i,k))*exp(-IU*ikx*i*pi/imx)) &
!                                   /ter(ns)*cn0s(ns)*gn0s(ns,i)/(IU*imx)
                      ! note IU is cmplx(0,1)
                      ! z1 = exp(IU*ikx*i*pi/imx)
                      ! cz1 = exp(-IU*ikx*i*pi/imx)
                      theta = ikx*i*pi/imx
                      z1 = dcmplx( cos(theta), sin(theta) )
                      cz1 = dconjg( z1 )

                      tmp=q(ns)*sin(ix*ikx*pi/imx)* &
                          ((1-gamb1(ns,ikx,j,i,k))*z1-&
                          (1-gamb2(ns,ikx,j,i,k))*cz1) &
                                   /ter(ns)*cn0s(ns)*gn0s(ns,i)/(IU*imx)
                      mx(i,ix,j,k)=mx(i,ix,j,k)+tmp
                      ! the matrix MX0 without adiabatic electron for the calcaulation of MX_zonal jycheng
                      mx0(i,ix,j,k)=mx0(i,ix,j,k)+tmp
                   enddo
                enddo
             enddo
          enddo
       enddo
    enddo
!$acc end parallel
!$acc end data
!   !$omp target update from(ter,theta,z1,cz1,mx,mx0)


    ! begin to calculate the MX_zonal jycheng
    mx_zonal=0.0

!$acc data copyin(jac(1:imx-1,0:1),mx0(:imx-1,:imx-1,:0,:)) &
!$acc& copyout(jacob_theta(:imx-1,:),mx_zonal(1:imx-1,1:imx-1,0,0:1))
!$acc parallel
!$acc loop gang vector collapse(3)
    do i=1,imx-1
       do k=0,1
          do ix=1,imx-1
             ! only consider the ky=0 part
             mx_zonal(i,ix,0,k)=mx0(i,ix,0,k)*jac(i,k)
             !jacob_theta(i,k)=jac(i,k)
          enddo
       !jacob_theta(i,k)=jac(i,k)
       enddo
    enddo
   
!$acc loop gang vector collapse(2)   
    do i=1,imx-1
       do k=0,1
          jacob_theta(i,k)=jac(i,k)
       enddo
    enddo
!$acc end parallel
!$acc end data
     ! flux average of mx0 !!!bad scaling: 192-200 takes 0.1s in 4 nodes, 0.26s in 16 nodes (htc)
     mx_tmp=0.0
     icount=(imx-1)*(imx-1)*jcnt*2        
     call MPI_ALLREDUCE(mx_zonal,mx_tmp,icount,MPI_DOUBLE_COMPLEX,MPI_SUM,TUBE_COMM,ierr)
     mx_zonal=mx_tmp
     jacob_theta_tmp=0.0
     icount=(imx-1)*2
     call MPI_ALLREDUCE(jacob_theta,jacob_theta_tmp,icount,MPI_REAL8,MPI_SUM,TUBE_COMM,ierr)

     jacob_theta=jacob_theta_tmp
!$acc data copy(mx_zonal(:,:,:,0:1)) copyin(jacob_theta(:,:))
!$acc parallel
!$acc loop vector collapse(4) 
     do i=1,imx-1
        do k=0,1
           do ix=1,imx-1
              do j=0,jcnt-1
                 mx_zonal(i,ix,j,k)=mx_zonal(i,ix,j,k)/jacob_theta(i,k)
              enddo
           enddo
        enddo
     enddo
!$acc end parallel
!$acc end data
     mx_zonal(:,:,0,:)=cmplx(real(mx_zonal(:,:,0,:)),0.0)
     mx(:,:,0,:)=cmplx(real(mx(:,:,0,:)),0.0)
!!!bad scaling: 215-256 taks 0.166s in 4 nodes, 0.303s in 16 nodes (htc)
     do k=0,1
        do j=0,jcnt-1
           call ZGETRF(imx-1,imx-1,mx(:,:,j,k),imx-1,ipiv(:,:,j,k),INFO )
           call ZGETRF(imx-1,imx-1,mx_zonal(:,:,j,k),imx-1,ipiv_zonal(:,:,j,k),INFO )
        enddo
     enddo
     
     ! save the matrix for restart
     if(iget.eq.0) then
        open(10000+MyId,file=fname,form='unformatted',status='unknown')
        write(10000+MyId)mx,mx_zonal,ipiv,ipiv_zonal
        close(10000+myid)
        goto 200
     end if
     
     ! write the mx,mx_zonal with ky=0,k=0
     if(gclr==kmx/2.and.tclr==0.and.nstep==0)then
       open(20,file="mx",status='unknown')
       j=0
       k=0
       do i=1,imx-1
          do ix=1,imx-1
             if(abs(i-ix)<40)write(20,10)i,ix,mx(i,ix,j,k),mx_zonal(ix,i,j,k)
          enddo
       enddo
10     format(1x,i5,1x,i5,2(2x,e12.5,2x,e12.5))
       close(20)
     end if

200  ifirst=-99
  endif
  call MPI_barrier(mpi_comm_world,ierr)
  !!! lines before this line are initialization
  poisson0_end_tm=poisson0_end_tm+MPI_WTIME()

  if(idg==1)write(*,*)'pass form factors'

  !now do field solve...

  ! calcaulte the zonal part of density
!$acc data copyout(rho_zonal_tmp(:im,:),jacob_rho_tmp(:im,:)) copyin(rho(0:im,0:jm-1,0:1),jac(0:im,0:1))
!$acc parallel
!$acc loop gang vector collapse(2)
  do i=0,im
     do k=0,1
        rho_zonal_tmp(i,k)=0.0
        jacob_rho_tmp(i,k)=0.0
        !$acc loop seq
        do j=0,jm-1
           rho_zonal_tmp(i,k)=rho_zonal_tmp(i,k)+rho(i,j,k)*jac(i,k)
           jacob_rho_tmp(i,k)=jacob_rho_tmp(i,k)+jac(i,k)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data

!!!allreduce takes a lot time, bad scaling: 270-282 take 0.144s in 4 nodes, 0.399 in 16 nodes
!start_allreduce=MPI_WTIME()
!if(myid==0)write(*,*)'before allreduce'
  rho_zonal=0.0
  icount=(im+1)*2
  call MPI_ALLREDUCE(rho_zonal_tmp,rho_zonal,icount,MPI_REAL8,MPI_SUM,mpi_comm_world,ierr)
  jacob_rho=0.0
  icount=(im+1)*2
  call MPI_ALLREDUCE(jacob_rho_tmp,jacob_rho,icount,MPI_REAL8,MPI_SUM,TUBE_COMM,ierr)
  do i=0,im
     do k=0,1
        rho_zonal(i,k)=rho_zonal(i,k)/jacob_rho(i,k)
     enddo
  enddo
!if(myid==0)write(*,*)'after allreduce',MPI_WTIME()-start_allreduce

  ! different source term for zonal and non-zonal term
!$acc data copyin(rho_zonal(:nxpp,:),rho(0:nxpp,0:jm,0:1)) copyout(u_zonal(:nxpp,:jm,:),u(:nxpp,:jm,:))
!$acc parallel
!$acc loop gang vector collapse(3)
  do i=0,nxpp
     do j=0,jm
        do k=0,1
           u(i,j,k)=rho(i,j,k)
           ! expand the zonal source term into x and y dimension
           u_zonal(i,j,k)=rho_zonal(i,k)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data


  ! expand the source term into sin tranformation
  call dcmpy(u(0:imx-1,0:jmx-1,0:1),v)
  call dcmpy(u_zonal(0:imx-1,0:jmx-1,0:1),v_zonal)

  temp3dxy=0.0
  ! solve the zonal-phi
!  do k=0,1
!     j=0
!     myj=jft(j)
!     sl(1:imx-1,j,k)=v_zonal(1:imx-1,j,k)
!     ! solve the MX_zonal 
!     call ZGETRS('N',imx-1,1,mx_zonal(:,:,j,k),imx-1,ipiv_zonal(:,:,j,k), &
!             sl(:,j,k),imx-1,INFO)
!     temp3dxy(1:imx-1,myj,k)=sl(1:imx-1,j,k)
!     temp3dxy(0,myj,k)=0.
!  enddo




 call init_batched()

  do k=0,1
     j=0
     myj=jft(j)
     sl(1:imx-1,j,k)=v_zonal(1:imx-1,j,k)

     strideA=(imx-1)*(imx-1)
     strideB=(imx-1)*1
     batchcount = 1
     sl_2dim(1:imx-1,1,batchcount) = sl(1:imx-1,j,k)
     c_trans ='N'
     mx_zonal_3dim(:,:,batchcount) = mx_zonal(:,:,j,k)
     ipiv_zonal_3dim(:,:,batchcount) = ipiv_zonal(:,:,j,k)
     
!     write(*,*),'mx_zonal_3dim dim1',size(mx_zonal_3dim,1),'mx_zonal_3dim dim2',size(mx_zonal_3dim,2),'mx_zonal_3dim dim3',size(mx_zonal_3dim,3)
!     write(*,*),'mx_zonal dim1',size(mx_zonal,1),'mx_zonal dim2',size(mx_zonal,2),'mx_zonal dim3',size(mx_zonal,3),'mx_zonal dim4',size(mx_zonal,4)
!     write(*,*),'ipiv_zonal_3dim dim1',size(ipiv_zonal_3dim,1),'ipiv_zonal_3dim dim2',size(ipiv_zonal_3dim,2),'ipiv_zonal_3dim dim3',size(ipiv_zonal_3dim,3)
!     write(*,*),'ipiv_zonal dim1',size(ipiv_zonal,1),'ipiv_zonal dim2',size(ipiv_zonal,2),'ipiv_zonal dim3',size(ipiv_zonal,3),'ipiv_zonal dim4',size(ipiv_zonal,4)
!     write(*,*),'sl_2dim dim1',size(sl_2dim,1),'sl_2dim dim2',size(sl_2dim,2),'sl_2dim dim3',size(sl_2dim,3)
!     write(*,*),'sl dim1',size(sl,1),'sl dim2',size(sl,2),'sl dim3',size(sl,3)

!    call init_batched()


     !$omp target enter data                                                             &
     !$omp& map(alloc:DINFO,ipiv_zonal_3dim(:,:,batchcount))                           &              
     !$omp& map(to:mx_zonal_3dim(:,:,batchcount),sl_2dim(:,1,batchcount))
    
      call ZgetrsStridedBatchedF(                                        &
     &      c_trans,imx-1,1,mx_zonal_3dim(:,:,batchcount),imx-1,strideA,ipiv_zonal_3dim(:,:,batchcount),                         &
      sl_2dim(:,1,batchcount),imx-1,strideB, DINFO, batchcount)
     
     !$omp target update from(DINFO)
     !$omp target update from(sl_2dim(:,batchcount))
     !$omp target exit data map(release:mx_zonal_3dim(:,:,batchcount),sl_2dim(:,batchcount),ipiv_zonal_3dim(:,:,batchcount),DINFO)

!     call finalize_batched()

     sl(:,j,k)=sl_2dim(:,batchcount) 
  
     temp3dxy(1:imx-1,myj,k)=sl(1:imx-1,j,k)
     temp3dxy(0,myj,k)=0.
  enddo

call finalize_batched()


  !  from rho(kx,ky) to phi(kx,ky)
!$acc data copy(temp3dxy(:,:,:))
!$acc parallel
!$acc loop gang vector collapse(3)
  do k = 0,1
     do j = 0,jmx-1
        do i = 0,imx-1
           temp3dxy(i,j,k)=temp3dxy(i,j,k)/jmx !*formphi(i,j,k)
       enddo
     enddo
  enddo
!$acc end parallel
!$acc end data

  !  from phi(kx,ky) to phi(x,y)
  do k=0,mykm
     do i=0,imx-1
        do j=0,jmx-1
          tmpy(j)=temp3dxy(i,j,k)
        enddo
        call ccfft('y',1,jmx,1.0,tmpy,coefy,worky,0)
        do j=0,jmx-1
           temp3dxy(i,j,k)=tmpy(j)  ! phi(x,y)
        enddo
     enddo
  enddo
 
  ! here phi is zonal-phi
!$acc data copyin(temp3dxy(:nxpp-1,:jm-1,:mykm)) copyout(phi(0:nxpp-1,0:jm-1,0:mykm))
!$acc parallel
!$acc loop gang vector collapse(3)
  do i=0,nxpp-1
     do j=0,jm-1
        do k=0,mykm
           phi(i,j,k)=temp3dxy(i,j,k)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data
  !$acc update host(phi)
  call enfz(phi(:,:,:))
  !$acc update device(phi)

  ! for the solver for ky=0 with ky!=0 part together, the difference is their source term, 
  ! for ky=0 term the source term is \delta n_i (ky=0) +<\phi>, for ky!=0 term, the source term is \delta n_i (ky!=0)
  ! the matrix is same, i.e. np-\phi (no matter k_y=0 or k_y!=0)
 
  ! the ky=0 source term
!$acc data present(phi) copyin(gn0e(0:nxpp),gt0e(0:nxpp),rho(0:nxpp,0:jm,0:1)) copyout(u_zonal(:nxpp,:jm,:))
!$acc parallel
!$acc loop gang vector collapse(3)
  do i=0,nxpp
     do j=0,jm
        do k=0,1
           u_zonal(i,j,k)=rho(i,j,k)+gn0e(i)*cn0e/gt0e(i)*phi(i,j,k)/real(ntube)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data
  call dcmpy(u_zonal(0:imx-1,0:jmx-1,0:1),v_zonal)

  temp3dxy=0.
  do k=0,1
     do j=0,jcnt-1
        myj=jft(j)
        ! the ky!=0 source term
        sl(1:imx-1,j,k)=v(1:imx-1,j,k)
        ! the ky=0 source term
        if(myj==jft(0))sl(1:imx-1,j,k)=v_zonal(1:imx-1,j,k)
        call ZGETRS('N',imx-1,1,mx(:,:,j,k),imx-1,ipiv(:,:,j,k), &
             sl(:,j,k),imx-1,INFO)
        temp3dxy(1:imx-1,myj,k)=sl(1:imx-1,j,k)
        temp3dxy(0,myj,k)=0.
     enddo
  enddo
  !  from rho(kx,ky) to phi(kx,ky)

!$acc data copy(temp3dxy(:,:,:))
!$acc parallel
!$acc loop gang vector collapse(3)
  do k=0,1
     do j=0,jmx-1
        do i=0,imx-1
           temp3dxy(i,j,k)=temp3dxy(i,j,k)/jmx !*formphi(i,j,k)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data

  !  from phi(kx,ky) to phi(x,y)
  do k=0,mykm
     do i=0,imx-1
        do j=0,jmx-1
           tmpy(j)=temp3dxy(i,j,k)
        enddo
        call ccfft('y',1,jmx,1.0,tmpy,coefy,worky,0)
        do j=0,jmx-1
           temp3dxy(i,j,k)=tmpy(j)  ! phi(x,y)
        enddo
     enddo
  enddo

!$acc data copyin(temp3dxy(:nxpp-1,:jm-1,:mykm)) copyout(phi(0:nxpp-1,0:jm-1,0:mykm))
!$acc parallel
!$acc loop gang vector collapse(3)
  do i=0,nxpp-1
     do j=0,jm-1
        do k=0,mykm
           phi(i,j,k)=temp3dxy(i,j,k)
        enddo
     enddo
  enddo
!$acc end parallel
!$acc end data
  !$acc update host(phi)
  !    x-y boundary points 
  call enfxy(phi(:,:,:))
  call enfz(phi(:,:,:))
  if(idg==1)write(*,*)'pass enfz', myid
  if(myid==0)then
    write(gemout,*)'end of gkps',phi(imx/2,jmx/2,0)
    flush(gemout)
  endif
  !$acc update device(phi)
  !$omp target update to(phi)
poisson2_end_tm=poisson2_end_tm+MPI_WTIME()
end subroutine gkps_adiabatic_electron

!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc







