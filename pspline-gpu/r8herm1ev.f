...
C
      ...
      intent(in) :: f, nx, x, ilinx, ict
      intent(inout) :: fval, ier
C
C       contents of f:
C
C  f(0,i) = f @ x(i)
...
c          on exit:  1.le.i.le.nx-1
      intent(in) :: xget,x,nx,ilinx
      intent(inout) :: i,ier
      intent(inout) :: xparam,hx,hxi
c
...
C
      ...
      intent(in) :: fin, hx,hxi,xparam
      intent(in) :: ii,ict,ivecd,nx,ivec
      intent(inout) :: fval
C
